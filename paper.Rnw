...
\abstract{\citet{Hoijtink:etal:inpress} present a method for choosing the prior distribution for a Bayes factor analysis that is based on controlling error rates, which they advocate as an alternative to our more subjective methods \citep{Rouder:etal:2009a,Wagenmakers:etal:2011,Morey:Rouder:2014}. We show that the method they advocate amounts to a simple significance test, and that the resulting Bayes factors are not interpretable. Additionally, their method fails in common circumstances, and has the potential to yield arbitrarily high Type~II error rates. After critiquing their method, we outline the position on subjectivity that underlies our advocacy of Bayes factors.}
...
\]
where $\mu$ and $\sigma^2$ are the mean and variance of the normal population, respectively. A so-called non-informative prior is placed on the nuisance parameter $\sigma^2$:
\[
...
Changing the parameter $\tau$ introduces subjective information into the test, as shown in Figure~\ref{fig:tvsBF}A: a small $\tau$ corresponds to the expectation that the effect is small, and a large $\tau$ corresponds to the expectation that the effect is large.  For a given $\tau$, the scaled-information Bayes factor indicates how much more likely the observed $t$ statistic would be under $H_0$ compared to $H_1$. This ratio is a measure of the relative strength of evidence provided by the data for $H_0$ over $H_1$. 

The logarithm of the scaled-information Bayes factor, in favor of the null hypothesis, is 
\begin{eqnarray}
...
\end{eqnarray}
The Bayes factor is a function of four variables: $t$, $N$, $\tau$, and the Bayes factor $BF$ itself. In a typical data analysis, $t$ and $N$ are provided by the data and $\tau$ is chosen {\em a priori}. When the resulting $\log(BF)$ is large and positive, the null hypothesis is preferred; when $\log(BF)$ is large and negative, the alternative is preferred. Note that the data affects the Bayes factor only through $t$ and $N$, and that for a given $N$ the Bayes factor is a monotone function of $|t|$: as the observed effect size increases, the Bayes factor will decrease, becoming more favorable to the alternative hypothesis.
...
\end{mydef}
As Figure~\ref{fig:tvsBF}B shows, however, the scaled information Bayes factor is simply a monotonic function of the $|t|$ statistic. Because the error rates in the test are determined solely by the critical $t$ statistic (which we denote $t_0$) or equivalently, the critical effect size (denoted $d_0$) used for the decision, HKH's Definition~1 is equivalent to ``the Bayes factor is well-calibrated if the critical $t_0$ statistic for the balanced-error significance test yields $BF_{01}=1$.'' This, in turn, is equivalent to:
...
Suppose we would like to test the null hypothesis  $H_0: \delta=\mu=0$ against a two-sided alternative $H_1: |\delta|=\delta_1$ for some positive $\delta_1$. It is important to note that this is not a typical significance test with alternative $\delta\neq0$; rather, it is a test of two simple point hypotheses, $\delta=0$ vs. $|\delta|=\delta_1$.  Under the specific alternative $\delta_1$, $d$ has a scaled noncentral $t$ distribution with noncentrality parameter $\delta_1\sqrt{N}$:
\[
d \sim \mbox{Noncentral}~t_{N-1}(\delta_1\sqrt{N})/\sqrt{N}.
\]

An example of this setup is shown in Figure~\ref{fig:simple} (top) for $\delta_1=.4$ and $N=36$. The critical bounds that determine when the test rejects are shown as vertical lines; in order to build a test with specified Type~I or Type~II error rates, it is only necessary adjust these bounds. 

<<simple,fig.cap="A frequentist two-sided test of $\\delta=0$ vs. $|\\delta|=.4$. The solid density is the distribution of $d$ under the null hypothesis; the left and right dashed densities are the distribution of the $d$ under the hypothesis $\\delta=-.4$ and $\\delta=.4$, respectively. Vertical lines show the critical bounds, beyond which (shaded region) the test will reject the null hypothesis. This particular choice for critical bounds yields balanced Type~I and Type~II errors. Upper: $N=36$; Lower: $N=180$.",fig.width=6,fig.height=8,echo=FALSE,warning=FALSE>>=
...
<<effSize,fig.cap="Distributions of effect sizes under the null (solid) and calibrated alternative (dashed) hypotheses. The vertical gray lines show the two-sided critical effect sizes where the Type~I and Type~II error rates are equal. Top: Sample size of $N=20$. Bottom: Sample size of $N=1000$.",fig.width=8,fig.height=8,echo=FALSE,warning=FALSE,eval=FALSE>>=
Ns = c(20, 1000)
min_d = -1
max_d = 1.5
...
HKH are not clear on how they expect researchers to implement calibrated analyses; however, understanding that the equal-error calibrated analysis is equivalent to a significance test of $\delta=0$ vs. $|\delta|=\delta_1$ reveals that an HKH-calibrated analysis for a given data set is completely determined by one of two settings: the stipulated error rates or the chosen $\delta_1$. Choosing either will determine $\tau$. In practice, therefore, HKH calibration must amount to choosing either $\alpha$ or $\delta_1$. Regardless of which the analyst chooses to set, the resulting Bayes factor has bad properties. We discuss both options in turn.

\subsubsection{Calibrating by choosing error rates}

In describing what they call the ``rational'' option for choosing $\tau$ (p. 13), HKH suggest that desired error rates can be used as a guide to calibration. They suggest choosing $\delta$ such that the error rates are neither too high nor too low.

Under Definition~1, choosing desirable error rates completely determines the alternative effect size $\delta_1$. The idea of choosing one's error rates and using them to choose an alternative is thinking backward: in both frequentist and Bayesian analysis, the analyst chooses an alternative first, based on substantive knowledge. The error rates are deductions based on the alternative and the sample size \citep[e.g.][]{Neyman:1957}. Although we believe choosing the error rates first is not defensible, we can examine the properties of the resulting Bayes factor under such a calibration scheme. The results here hold for both of HKH's definitions of calibration.

Because the precision with which $\delta$ is estimated increases as the sample size $N$ increases, in order to obtain a fixed-error significance test as $N\rightarrow\infty$, the alternative effect size $\delta_1$ and the critical effect size $\pm d_0$ must decrease and approach 0. As $\delta_1\rightarrow0$, the alternative is more and more similar to the null hypothesis; to accommodate this, the calibrated $\tau$ must also approach 0. Figure~\ref{fig:BFN1}A shows how the calibrated $\tau$ approaches 0 as $N\rightarrow\infty$ assuming that the desired Type~I error rate is $\alpha=.2$.

<<BFN1,fig.cap="A: The HKH-calibrated prior scale $\\tau$ as a function of sample size. B: HKH-calibrated (solid line) and scaled-information ($\\tau=1$; dashed line) Bayes factors for an observed effect size of $d=0$ as a function of sample size.  In both plots, HKH-calibration is performed against Type~I error rate $\\alpha=.2$, and holds for both definitions of calibration.",fig.width=8,fig.height=4, echo=FALSE>>=
...
The ever-decreasing calibrated $\tau$, in turn, induces strange behaviors in the Bayes factor. Consider that a basic desiderata of a Bayes factor analysis is that the most null observation $d=0$ should yield an increasing amount of evidence for the null hypothesis as $N\rightarrow\infty$: a completely null effect size should be more convincing with 1000 participants than with 100. The scaled information Bayes factor has this property, as shown in Figure~\ref{fig:BFN1}B (solid line). The HKH-calibrated Bayes factor, however, does not. The null effect size $d=0$ yields {\em decreasing} amounts of evidence for the null hypothesis as $N\rightarrow\infty$, due to the fact that $\tau\rightarrow0$. Figure~\ref{fig:BFN1}B (dashed line) shows that the Bayes factor for $d=0$ continually decreases, approaching a constant $BF<2$ as $N$ grows. 
...
HKH also suggest that it is possible to translate prior knowledge about effect sizes into an ``optimal'' $\tau$. For instance, they note that if one is looking for an effect size between .2 and .3, one could choose $\tau$ such that a test against $\delta_1=.25$ yields equal error rates (p. 12). In line with HKH's suggestion, we can examine the properties of the calibrated Bayes factor when we select a reasonable {\em a priori} $\delta_1$ against which to calibrate and then choose the $\tau$ according to their Definition~1.

In order to see the problem with calibration against a fixed $\delta_1$, the following fact is helpful: as $N\rightarrow\infty$, if we keep the error rates balanced as suggested under Definition~1, then the critical bounds $\pm d_0$ will approach $\pm \delta_1/2$ (proof provided in the supplemental materials). Figure~\ref{fig:simple} (bottom) shows the significance test for $\delta_1=.4$ and $N=180$, and indeed, the critical bounds $\pm d_0$ are very close to half-way between the two hypotheses. This makes sense: for a test of $\delta=0$ against $|\delta|=\delta_1$, the null hypothesis is preferred when $d$ is closer to 0; otherwise, $|\delta_1|=\delta_1$ is preferred.
...
To see this, suppose we perform a HKH test calibrated against $\delta_1=\Sexpr{d1}$, and consider the Bayes factor for $d=\Sexpr{round(delta_hat,2)}$; that is, we observe an effect size just under half the calibrated effect size. We choose $\delta_1=\Sexpr{d1}$ for demonstration, and because HKH used it in their figures; any other value of $\delta_1$ would suffice.
...
<<BFN2,fig.cap="A: HKH-calibrated (solid line) and scaled-information ($\\tau=1$; dashed line) Bayes factors for an observed effect size of $d=.19$ as a function of sample size. B: The HKH-calibrated prior scale $\\tau$ as a function of sample size. In both plots, HKH-calibration is performed against $\\delta_1=0.4$.",fig.width=8,fig.height=4, echo=FALSE>>=
...
This strange behavior is caused by the fact that the prior scale $\tau$ must be increased to bias the analysis increasingly toward the null hypothesis in order to accommodate the unreasonable requirement that an observed effect size of $d=\Sexpr{d1/2}$ yield equivocal evidence as $N\rightarrow\infty$. Figure~\ref{fig:BFN2}B shows the increase in the prior scale as a function of the sample size.  At a sample size of 500, the HKH-calibrated scale is well over 1000. To put this in perspective, a prior scale of 1000 means that a priori, true effect sizes of $\delta>100$ --- several orders of magnitude larger than those actually seen in experiments --- are expected with 12 to 1 odds. Such an extreme prior is needed to yield such an extreme result. The prior is so extreme, in fact, that we do not see how it can be called a ``prior'' at all. The resulting Bayes factor is uninterpretable.

Another way to view this is from a frequentist perspective. Ignoring the Bayes factor calibration and simply focusing on the equal-error significance test, if the true $\delta$ is less than one-half the calibrated $\delta_1$, then the probability of a Type~II error --- as defined as making a decision for $H_0$ when $\delta\neq0$ --- approaches 1 as $N\rightarrow\infty$. Consequently, the power of the test goes to 0. Only if the true $\delta$ is closer to $-\delta_1$ or $\delta_1$ will the power of the test increase as sample size increases. This is why no one uses significance tests with point alternatives in practice.
...
Suppose our researchers obtain an observed effect size of $d=\Sexpr{d}$ ($SE_d=\Sexpr{round(1/sqrt(N),3)}, t_{\Sexpr{N-1}}=\Sexpr{round(d*sqrt(N),3)},p<.001$). Using the scaled information Bayes factor with a $\tau=\Sexpr{round(tau_sub,3)}$  chosen using the prior studies (yielding a 50\% probability that $|\delta|<\Sexpr{d1}$)\footnote{In this situation, we would argue that a one-sided test with this $\tau$ test is appropriate. However, the question of one-sided vs. two-sided test is irrelevant to the point we are making; to remain consistent with the reported $t$ test and the HKH calibrated test, we report a two-sided Bayes factor.}, we obtain a Bayes factor of $BF_{10}=\Sexpr{round(bfs_scaled_info,0)}$. This Bayes factor favors the alternative hypothesis, as would be expected from the moderately large $t$ value. The HKH calibrated Bayes factor in favor of the alternative, on the other hand, is $BF_{HKH}=\Sexpr{round(bfs_calibrated,2)}$. Because this is less than 1, the researchers would decide in favor of the null hypothesis, in spite of having evidence {\em against} it, simply because the observed effect size was $d<\delta_1/2$.

\subsection{Problems with small $\tau$ values}

More basic problems with HKH-calibration exist when the calibrated $\tau$ is small. Consider calibrating a Bayes factor when $N=36$, against the alternatives $\delta_1=.2$ and $\delta_1=.25$. This sample size and these effect sizes were used by HKH to demonstrate the calibration method. 
...
For $\delta_1=.2$ and $N=36$, the only $\tau$ that meets HKH's calibration Definition~1 is $\tau=0$, as shown in Figure~\ref{fig:smallTau}A. Recall that $\tau$ is the standard deviation of the Normal prior under $H_1$. If $\tau=0$, then $H_1$ is represented by a Normal with mean 0 and standard deviation 0; that is, the alternative $H_1$ is exactly the same as the null $H_0$. If the null and alternative are identical, {\em all data} will yield a Bayes factor of 1. In fact, whenever the critical $t_0$ for the HKH-calibrated Bayes factor is $|t|\leq1$ the HKH-calibrated Bayes factor always exactly 1. Figure~\ref{fig:smallTau}B shows for which combinations of $\delta_1$ and $N$ this occurs; for all values in the shaded region, the HKH-calibrated Bayes factor will equal 1, regardless of the data.
